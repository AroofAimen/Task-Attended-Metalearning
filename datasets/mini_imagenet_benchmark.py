...
def mini_imagenet_tasksets(
    ...
    **kwargs,
    ...
    if(memorization):
        # print("memorization happening")
        train_transforms = [
            XNWays(train_dataset, train_ways),
            KShots(train_dataset, train_samples),
            LoadData(train_dataset),
            XRemapLabels(train_dataset)
        ]
       
        valid_transforms = [
            XNWays(valid_dataset, test_ways),
            KShots(valid_dataset, test_samples),
            LoadData(valid_dataset),
            XRemapLabels(valid_dataset),
        ]
        test_transforms = [
            XNWays(test_dataset, test_ways),
            KShots(test_dataset, test_samples),
            LoadData(test_dataset),
            XRemapLabels(test_dataset)
        ]
    else:
        train_transforms = [
